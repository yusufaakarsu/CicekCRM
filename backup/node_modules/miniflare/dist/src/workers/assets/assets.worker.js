...
var AssetsManifest = class {
  ...
  if (arr.byteLength === 0)
    return !1;
  let low = 0, high = arr.byteLength / 40 - 1;
  if (high === low) {
    let current = new Uint8Array(arr.buffer, arr.byteOffset, 16);
    if (current.byteLength !== searchValue.byteLength)
      throw new TypeError(
        "Search value and current value are of different lengths"
      );
    return compare(current, searchValue) === 0 ? new Uint8Array(arr.buffer, arr.byteOffset, 40) : !1;
  }
  let searchValueNumber = uint8ArrayToNumber(searchValue);
  for (; low <= high; ) {
    let lowValue = new Uint8Array(
      ...
      Number(
        BigInt(low) + BigInt(high - low) * (searchValueNumber - uint8ArrayToNumber(lowValue)) / (uint8ArrayToNumber(highValue) - uint8ArrayToNumber(lowValue))
      )
...
var src_default = class extends WorkerEntrypoint {
  ...
  async unstable_exists(pathname) {
    let analytics = new ExperimentAnalytics(this.env.EXPERIMENT_ANALYTICS), performance = new PerformanceTimer(this.env.UNSAFE_PERFORMANCE), INTERPOLATION_EXPERIMENT_SAMPLE_RATE = 0, searchMethod = "binary";
    ...
    try {
      ...
      return searchMethod === "interpolation" ? await assetsManifest.getWithInterpolationSearch(pathname) : await assetsManifest.getWithBinarySearch(pathname);
    } finally {
